"""Evaluate BFN, DDPM, DDIM, Consistency policies on Catch Point environment."""
...
print("Starting Catch Point eval script...", flush=True)
...
NUM_DISCRETE = 2
ACTION_DIM_CONT = NUM_DISCRETE + 1  # 2 one-hot + 1 angle = 3
...
def load_policy(checkpoint_path: str, policy_type: str, device: str = "cuda"):
    ...
    if policy_type == "bfn":
        ...
        policy = ConsistencyLowdimPolicy(
            ...
            ctm_weight=1.0, dsm_weight=1.0, delta=0.0,
            teacher_path=None,
        )
...
def main():
    ...
    if len(results) >= 2:
        print(f"\n{'='*70}")
        print(f"Comparison Summary (Catch Point, 2 actions)")
        print(f"{'='*70}")
